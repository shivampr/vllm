...
class PrometheusStatLogger(AggregateStatLoggerBase):
    ...
    def __init__(
        ...
        histogram_kv_block_lifetime = self._histogram_cls(
            ...
            documentation="Histogram of KV cache block lifetime from allocation to eviction.",
        ...
        histogram_kv_block_reuse_gap = self._histogram_cls(
            ...
            documentation="Histogram of time gaps between consecutive KV cache block accesses.",
            buckets=kv_cache_residency_buckets,
            labelnames=labelnames,
        )
        ...
        histogram_request_prefix_residency = self._histogram_cls(
            ...
            documentation="Histogram of request prefix block residency time after prefill completion.",
            buckets=kv_cache_residency_buckets,
            labelnames=labelnames,
        )
        ...
        if vllm_config.cache_config.kv_cache_metrics:
            for engine_idx in engine_indexes:
                collector = KVCacheMetricsCollector(
                    enabled=True,
                    sample_rate=vllm_config.cache_config.kv_cache_metrics_sample,
                )
                collector.histogram_block_lifetime = self.histogram_kv_block_lifetime[engine_idx]
                collector.histogram_idle_before_evict = self.histogram_kv_block_idle_before_evict[engine_idx]
                collector.histogram_reuse_gap = self.histogram_kv_block_reuse_gap[engine_idx]
                collector.histogram_prefix_residency = self.histogram_request_prefix_residency[engine_idx]
                self.kv_cache_metrics_collectors[engine_idx] = collector

    def get_kv_cache_metrics_collector(self, engine_idx: int = 0) -> KVCacheMetricsCollector | None:
