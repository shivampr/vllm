...
class BlockMetricsState:
    ...
    __slots__ = ('birth_time_ns', 'last_access_ns', 'access_history', 'max_request_end_ns')
    
    def __init__(self):
        now_ns = time.monotonic_ns()
        self.birth_time_ns = now_ns
        self.last_access_ns = now_ns
        self.access_history = deque(maxlen=4)  # Ring buffer for reuse gaps
        self.max_request_end_ns = 0  # For prefix residency tracking
...
class KVCacheMetricsCollector:
    ...
    def __init__(self, enabled: bool = False, sample_rate: float = 0.01):
        ...
        if sample_rate <= 0:
            ...
            logger.warning("kv_cache_metrics_sample %f > 1.0, clamping to 1.0", sample_rate)
    ...
    def on_block_evicted(self, block: "KVCacheBlock") -> None:
        ...
        with self._lock:
            ...
            if self.histogram_block_lifetime:
                self.histogram_block_lifetime.observe(metrics.get_lifetime_seconds(now_ns))
            
            if self.histogram_idle_before_evict:
                self.histogram_idle_before_evict.observe(metrics.get_idle_time_seconds(now_ns))
    ...
    def on_request_prefill_complete(self, request_id: str, prefix_block_ids: set[int]) -> None:
